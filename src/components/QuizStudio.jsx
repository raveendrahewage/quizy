      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-200">
        ...
        <div className="px-8 py-6 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">Quiz Studio</h2>
              <p className="text-slate-500 font-bold text-sm">Create and manage your own quizzes.</p>
            </div>
          </div>
          <button onClick={onClose} className="p-3 bg-white hover:bg-slate-100 rounded-xl border border-slate-200 transition-colors shadow-sm">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar: Quiz List */}
          <div className="w-1/3 border-r border-slate-200 bg-slate-50 flex flex-col overflow-hidden shrink-0">
            <div className="p-6 border-b border-slate-200 bg-white">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Create New Quiz</h3>
              <div className="space-y-3">
                <input
                  value={newQuizData.title}
                  onChange={(e) => setNewQuizData({ ...newQuizData, title: e.target.value })}
                  placeholder="Quiz Title"
                  className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none font-bold text-slate-800 transition-all"
                />
                <input
                  value={newQuizData.description}
                  onChange={(e) => setNewQuizData({ ...newQuizData, description: e.target.value })}
                  placeholder="Short Description"
                  className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none font-medium text-sm text-slate-800 transition-all"
                />
                
                <label className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl cursor-pointer hover:bg-white transition-colors">
                  <input
                    type="checkbox"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm text-slate-700">{newQuizData.isPrivate ? "Private Mode" : "Public Mode"}</span>
                  </div>
                </label>
                ...
                {newQuizData.isPrivate && (
                  <input
                    ...
                    maxLength={10}
                    className="w-full p-3 rounded-xl border border-amber-200 bg-amber-50 focus:bg-white focus:border-amber-500 outline-none font-bold text-slate-800 transition-all placeholder:text-amber-300"
                  />
                )}

                <button
                  onClick={handleCreateQuiz}
                  className="w-full py-3 bg-slate-900 hover:bg-indigo-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-colors"
                >
              ...
              {quizzes.map((q) => (
                <div
                  ...
                  className={`
                    ...
                    ${activeQuiz?.id === q.id ? "bg-indigo-50 border-indigo-500" : "bg-white border-transparent hover:border-slate-200 shadow-sm"}
                  `}
                >
                  <div className="overflow-hidden pr-3 flex items-center gap-2">
                    {q.isPrivate && <Lock size={14} className="text-amber-500 shrink-0" />}
                    <h4 className={`font-black truncate ${activeQuiz?.id === q.id ? "text-indigo-900" : "text-slate-700"}`}>{q.title}</h4>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button 
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${activeQuiz?.id === q.id ? 'bg-indigo-100 text-indigo-600 hover:bg-indigo-200' : 'bg-slate-100 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'}`}
                      title="Copy Link"
                    >
                      <Link size={14} />
                    </button>
            ...
            <div className="p-4 border-t border-slate-200 bg-white mt-auto">
              <button 
                ...
                className="w-full py-2 bg-amber-100 hover:bg-amber-200 text-amber-800 rounded-xl font-bold text-xs transition-colors flex items-center justify-center gap-2"
              >
                Migrate Legacy Data
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-white p-8">
            {!activeQuiz ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400">
                <Edit3 size={64} className="mb-4 text-slate-200" />
              ...
              <div className="max-w-3xl mx-auto">
                <div className="flex justify-between items-start mb-10 pb-6 border-b border-slate-100">
                  <div>
                    <h2 className="text-3xl font-black text-slate-800 mb-2">{activeQuiz.title}</h2>
                    <p className="text-slate-500 font-medium">{quizQuestions.length} Questions configured</p>
                  </div>
                  <button 
                ...
                <div className="grid lg:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-black">1</div>
                      <h3 className="font-black text-lg text-slate-800">Add Question</h3>
                    </div>
                    ...
                    <div className="space-y-4">
                      <textarea
                        ...
                        className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none font-bold text-slate-800 min-h-[100px] resize-y"
                      />
                      ...
                      <label className="flex items-center gap-3 p-4 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                        <input
                          ...
                          className="w-5 h-5 accent-indigo-600 rounded"
                        />
                        <span className="font-bold text-slate-700">Multiple correct answers?</span>
                      </label>

                      <div className="space-y-3">
                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Answers (Check the correct ones)</p>
                        {newQ.options.map((opt, i) => (
                          <div key={i} className={`flex gap-3 items-center rounded-xl p-2 border-2 transition-colors ${newQ.correct.includes(i) ? 'border-emerald-500 bg-emerald-50' : 'border-slate-100 bg-white'}`}>
                            <div className="font-black text-slate-300 w-6 text-center">{String.fromCharCode(65 + i)}</div>
                            <input
                              value={opt}
                              onChange={(e) => {
                              ...
                              className="flex-1 p-2 bg-transparent outline-none font-bold text-slate-700"
                            />
                            <button
                              onClick={() => toggleCorrect(i)}
                              className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-colors shadow-sm ${newQ.correct.includes(i) ? "bg-emerald-500 text-white" : "bg-white border text-slate-200 hover:border-emerald-200 hover:text-emerald-500"}`}
                            >
                              <Save size={18} />
                            </button>
                          </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-6">
                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-black">2</div>
                      <h3 className="font-black text-lg text-slate-800">Current Questions Focus</h3>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {quizQuestions.map((q, i) => (
                        <div key={q.id} className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm group hover:border-indigo-300 transition-colors relative">
                          <div className="flex justify-between items-start gap-4">
                            <div>
                              <div className="text-xs font-black text-indigo-500 mb-1">Question {i + 1}</div>
                              <h4 className="font-bold text-slate-800 leading-tight">{q.text}</h4>
                            </div>
                            <button
                          ...
                          <div className="mt-3 flex gap-2 flex-wrap">
                            {q.correct.map(cIdx => (
                              <span key={cIdx} className="text-[10px] font-black uppercase text-emerald-700 bg-emerald-100 px-2 py-1 rounded">
