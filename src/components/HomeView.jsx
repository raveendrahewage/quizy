...
export default function HomeView({ user, onPlayQuiz }) {
  ...
  useEffect(() => {
    const visited = localStorage.getItem("has_visited_quizy");
    if (!visited) {
      setIsFirstVisit(true);
      localStorage.setItem("has_visited_quizy", "true");
    }
  }, []);
  ...
  useEffect(() => {
    if (quizzes.length > 0 && !checkedUrlRef.current) {
      ...
      if (quizId) {
        ...
        if (targetQ) {
          if (targetQ.isPrivate && targetQ.creatorId !== user.uid) {
  ...
  const handlePlayClick = (quiz) => {
    if (quiz.isPrivate && quiz.creatorId !== user.uid) {
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-12 relative min-h-screen overflow-hidden">
      ...
      <div className="mb-16 relative z-10">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full font-bold text-sm mb-6 border border-indigo-100 dark:border-indigo-800/50 shadow-sm"
        >
        ...
        <motion.p 
          ...
          className="mt-6 text-slate-500 dark:text-slate-400 font-medium text-xl max-w-2xl leading-relaxed"
        >
        ...
        <AnimatePresence>
          {quizzes.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-2xl font-black text-slate-700 dark:text-slate-300 mb-2">No Quizzes Yet</h3>
              <p className="text-slate-500 dark:text-slate-500 font-medium text-lg">Check back later or ask an admin to create some!</p>
            </motion.div>
          ) : (
            quizzes.map((quiz, i) => (
              <motion.div
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-6">
                    <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors flex items-center gap-3">
                      {quiz.isPrivate && <Lock size={24} className="text-amber-500 shrink-0" />}
                      {quiz.title}
                    </h3>
                  </div>
                  
                  <p className="text-slate-500 dark:text-slate-400 font-medium mb-10 text-lg leading-relaxed line-clamp-2">
                  ...
                  <div className="mt-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 items-center justify-between">
                      <div className="flex gap-4 w-full sm:w-auto">
                        <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300 font-bold bg-white dark:bg-slate-800 px-4 py-2.5 rounded-xl border-2 border-slate-100 dark:border-slate-700 shadow-sm">
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto px-8 py-3.5 bg-slate-900 dark:bg-white dark:text-slate-900 hover:bg-indigo-600 dark:hover:bg-indigo-400 text-white rounded-2xl font-black shadow-xl shadow-slate-900/20 dark:shadow-indigo-500/10 hover:shadow-indigo-600/40 transition-all duration-300 hover:-translate-y-1 flex items-center justify-center gap-3 overflow-hidden relative overflow-group group/btn"
                      >
                        <span className="relative z-10">Play Now</span>
                        <Play size={18} fill="currentColor" className="relative z-10 group-hover/btn:translate-x-1 transition-transform" />
                        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-violet-600 dark:from-indigo-400 dark:to-violet-500 opacity-0 group-hover/btn:opacity-100 transition-opacity z-0" />
                      </button>
                    </div>
                    ...
                    {leaderboards[quiz.id] && leaderboards[quiz.id].length > 0 && (
                      <div className="mt-8 pt-6 border-t border-slate-200/60 dark:border-slate-800/60">
                        <div className="flex items-center gap-2 mb-4 text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                          <BarChart3 size={14} className="text-indigo-400 dark:text-indigo-500" /> Current Leaders
                        </div>
                        <div className="flex -space-x-4">
                          {leaderboards[quiz.id].map((lb, idx) => (
                            <div key={idx} className="relative group/avatar cursor-pointer">
                              ...
                              />
                              <div className="absolute opacity-0 group-hover/avatar:opacity-100 bg-slate-800 dark:bg-slate-700 text-white text-xs font-black px-4 py-2 rounded-xl -top-12 left-1/2 -translate-x-1/2 whitespace-nowrap transition-all duration-300 shadow-xl pointer-events-none z-30 scale-95 group-hover/avatar:scale-100">
                                {lb.name}
                                <span className="text-indigo-400 dark:text-indigo-300 ml-2">{lb.score}</span>
        ...
        {pinPromptQuiz && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-2xl w-full max-w-sm relative border border-white dark:border-slate-800"
            >
              <button 
                onClick={() => setPinPromptQuiz(null)}
                className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors"
              >
                <X size={20} />
              </button>
              ...
              <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100 text-center mb-2">Private Quiz</h3>
              ...
              <input
                ...
                autoFocus
                className={`w-full p-4 text-center text-xl font-black tracking-widest rounded-xl border-2 outline-none transition-all mb-4 text-slate-800 dark:text-slate-100 ${pinError ? 'border-red-500 bg-red-50 dark:bg-red-950/30 focus:border-red-600' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 focus:bg-white dark:focus:bg-slate-800 focus:border-indigo-500'}`}
              />
