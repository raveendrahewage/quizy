...
export default function QuizPlayer({ user, quiz, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-8">
      <button 
        onClick={onBack}
        className="flex items-center gap-2 text-slate-500 font-bold hover:text-indigo-600 transition-colors mb-8 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 w-fit"
      >
      ...
      <div className="max-w-6xl mx-auto grid lg:grid-cols-3 gap-8">
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -20 }}
                className="bg-white p-10 md:p-16 rounded-[3rem] shadow-xl shadow-slate-200/40 border border-slate-100 text-center relative overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />
                
                <h1 className="text-4xl md:text-5xl font-black mb-6 text-slate-800 tracking-tight">
                  {quiz.title}
                </h1>
                
                {questions.length === 0 ? (
                  <div className="p-8 bg-amber-50 text-amber-800 rounded-3xl border border-amber-200 font-bold text-lg inline-block">
                    This quiz has no questions yet!
                  </div>
                ) : (
                  <>
                    <p className="text-slate-500 mb-10 text-xl max-w-lg mx-auto leading-relaxed">
            ...
            {step === 1 && (
              <motion.div 
                ...
                className="bg-white rounded-[3rem] shadow-xl shadow-slate-200/40 border border-slate-100 relative overflow-hidden flex flex-col min-h-[500px]"
              >
                {/* Progress bar */}
                ...
                <div className="p-10 flex-1 flex flex-col">
                  ...
                  <div className="flex justify-between items-center mb-10 mt-4">
                    <div className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-black shadow-sm transition-colors border ${
                      timeLeft <= 5 ? 'bg-red-50 text-red-600 border-red-200' : 'bg-slate-50 text-slate-700 border-slate-200'
                    }`}>
                      <Clock size={20} className={timeLeft <= 5 ? "animate-pulse" : ""} /> 
                      <span className="text-xl">{timeLeft}s</span>
                    </div>
                    <div className="text-sm font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-full border border-slate-100">
                  ...
                  <div className="relative mb-10">
                    {questions[currentQ].isMulti && (
                      <div className="absolute -top-10 left-0 flex items-center gap-1.5 text-xs font-black uppercase text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 shadow-sm">
                      ...
                      return (
                        <button
                          ...
                          className={`
                            ...
                            ${isSelected 
                              ? "border-indigo-600 bg-indigo-50 text-indigo-800 shadow-md shadow-indigo-100 translate-x-2" 
                              : "border-slate-100 bg-white text-slate-600 hover:border-indigo-300 hover:bg-slate-50 hover:shadow-sm"
                            }
                          `}
                        >
                          {questions[currentQ].isMulti ? (
                            <div className={`
                              ...
                              ${isSelected ? "text-indigo-600 bg-white shadow-sm" : "text-slate-300 bg-slate-100 group-hover:bg-slate-200"}
                            `}>
                              {isSelected ? <CheckSquare size={24} className="fill-current text-white stroke-indigo-600" /> : <Square size={24} />}
                            </div>
                          ) : (
                            <div className={`
                              w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors
                              ${isSelected ? "border-indigo-600 bg-indigo-600 text-white" : "border-slate-200 text-slate-400 group-hover:border-indigo-300"}
                            `}>
                  ...
                  {questions[currentQ].isMulti && (
                    <motion.button
                      ...
                      onClick={handleNext}
                      className="w-full mt-8 py-5 bg-slate-900 hover:bg-indigo-600 text-white rounded-2xl font-black shadow-lg shadow-slate-900/20 transition-all text-lg"
                    >
                      Submit Selection
                    </motion.button>
                  )}
            ...
            {step === 2 && (
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1 }}
                className="bg-white p-12 rounded-[3rem] text-center shadow-xl shadow-slate-200/40 border border-slate-100 relative overflow-hidden"
              >
                ...
                <h2 className="text-5xl font-black mb-4 text-slate-800">Quiz Complete!</h2>
                <div className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600 my-8">
                  {score} <span className="text-2xl text-slate-400 uppercase tracking-widest font-bold">pts</span>
                </div>
                <div className="flex gap-4 justify-center mt-10 relative z-10">
                  <button
                    ...
                    className="px-8 py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl font-bold transition-colors"
                  >
                    Play Again
                  </button>
                  <button
        ...
        {/* Sidebar Leaderboard */}
        <div className="bg-white p-8 rounded-[3rem] shadow-xl shadow-slate-200/40 border border-slate-100 h-fit lg:sticky lg:top-8">
          <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-100 text-slate-800">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 font-bold">{quiz.title}</p>
            </div>
          </div>
                ...
                <motion.div
                  ...
                  className={`
                    ...
                    ${i === 0 ? "bg-amber-50 border-amber-200 shadow-sm" : 
                      ...
                      i === 2 ? "bg-orange-50/50 border-orange-100" :
                      "bg-transparent border-transparent hover:border-slate-100"
                    }
                  `}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <img
                      ...
                      alt="avatar"
                    />
                    <span className="font-bold text-slate-700 truncate w-24">
                      {entry.name}
                    </span>
                  </div>
                  <span className={`font-black text-lg ${i === 0 ? "text-amber-600" : "text-indigo-600"}`}>
                    {entry.score}
                  </span>
                </motion.div>
              ))
