...
import { Trophy, LogOut, Settings, Moon, Sun } from "lucide-react";
...
export default function App() {
  ...
  if (!user) {
    return (
      <div className={`min-h-screen flex items-center justify-center p-6 font-sans relative overflow-hidden transition-all duration-1000 ${isDark ? 'bg-[#05070a]' : 'bg-[#f0f4f8]'}`}>
        <div className="absolute top-8 right-8 flex gap-4 z-50">
          <button
            onClick={() => setIsDark(!isDark)}
            className={`p-3.5 rounded-2xl border-2 transition-all flex items-center gap-3 font-black shadow-lg hover:scale-105 active:scale-95 ${isDark ? 'bg-slate-900/60 text-white border-slate-800' : 'bg-white text-slate-700 border-slate-100'}`}
          >
        ...
        <div className={`absolute -top-24 -left-24 w-[35rem] h-[35rem] rounded-full blur-[120px] mix-blend-multiply opacity-50 animate-float ${isDark ? 'bg-indigo-900/40' : 'bg-indigo-200'}`}></div>
        <div className={`absolute top-1/2 -right-24 w-[30rem] h-[30rem] rounded-full blur-[120px] mix-blend-multiply opacity-50 animate-float-delayed ${isDark ? 'bg-violet-900/40' : 'bg-violet-200'}`}></div>
        <div className={`absolute -bottom-24 left-1/2 w-[25rem] h-[25rem] rounded-full blur-[100px] mix-blend-multiply opacity-30 animate-pulse-slow ${isDark ? 'bg-pink-900/30' : 'bg-pink-100'}`}></div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans selection:bg-indigo-200 transition-colors duration-300">
      {/* Admin Panel Overlay */}
      <AnimatePresence>
      ...
      {/* Navigation */}
      <nav className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700">
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 font-bold rounded-xl transition-all shadow-sm flex items-center gap-2 border border-indigo-100 dark:border-indigo-800/50"
              title="Quiz Studio"
            >
              <Settings size={18} />
